...
from auth import get_current_user, role_checker, router as auth_router
...
@app.get(
    ...
    dependencies=[Depends(role_checker([UserRole.admin, UserRole.user, UserRole.moderator]))]
)
...
@app.post("/pokemon/load", dependencies=[Depends(role_checker([UserRole.admin]))]

          )
def fetch_and_store(db: Session = Depends(get_db)):
    response = requests.get("https://coralvanda.github.io/pokemon_data.json")
    if response.status_code != 200:
        raise HTTPException(status_code=response.status_code, detail="Failed to load data.")
    data = response.json()
